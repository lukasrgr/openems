...
@Component({
  selector: HeatingElementRTUComponent.SELECTOR,
  templateUrl: './heatingelementrtu.component.html'
})
export class HeatingElementRTUComponent {
  ...
  private static readonly SELECTOR = "heatingElementRTU";
  ...
  private subscribeOnAddedComponents() {
    ...
    this.edge.subscribeChannels(this.websocket, 'heatingElementRTU', this.subscribedChannels);
    setTimeout(() => {
      this.loading = false;
      this.running = true;
    }, 5000);
  ...
  ionViewDidLeave() {
    this.edge.unsubscribeChannels(this.websocket, 'heatingElementRTU');
